...
{
    m_meshData.resize(pScene->mNumMeshes);
...
void Mesh::InitMesh(UINT Index, const aiMesh* paiMesh)
{
    ...
    for (UINT i = 0; i < paiMesh->mNumVertices; i++)
    {
        ...
        m_meshData[i].Vertices.push_back(v);
    }

    for (UINT i = 0; i < paiMesh->mNumFaces; i++)
    {
        const aiFace& Face = paiMesh->mFaces[i];
        m_meshData[i].Indices.push_back(Face.mIndices[0]);
        m_meshData[i].Indices.push_back(Face.mIndices[1]);
        m_meshData[i].Indices.push_back(Face.mIndices[2]);
    }
}
